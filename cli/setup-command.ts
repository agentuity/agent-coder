...
async function detectExistingConfig(): Promise<Partial<ConfigData> | null> {
  ...
  try {
    ...
    try {
      ...
      console.log(
        chalk.yellow(
          '📁 Existing local configuration found (will migrate to global)!'
        )
      );
      return config;
    } catch {
...
async function askQuestions(
  ...
  const allowedCommands = answers.allowedCommands.flatMap(
    (type: string) => commandMap[type] || []
  );
...
`;
        ...
        console.log(
          chalk.green(
            `✅ Global CLI installed to: ${path.join(targetDir, 'coder.bat')}`
          )
        );
...
async function validateSetup(config: ConfigData): Promise<void> {
  ...
  try {
    ...
    console.log(
      chalk.dim(`  ✓ Agent URL detected: ${testUrl.substring(0, 50)}...`)
    );
