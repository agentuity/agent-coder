...
function getGlobalConfigPath(): string {
	return join(homedir(), '.config', 'agentuity-coder', 'agentuity-coder.config.json');
}

function getGlobalConfigDir(): string {
	return join(homedir(), '.config', 'agentuity-coder');
}
...
async function detectExistingConfig(): Promise<Partial<ConfigData> | null> {
	...
	try {
		...
		console.log(chalk.yellow('📁 Existing global configuration found!'));
		return config;
	} catch {
		// Check legacy local config
		try {
			await access('agentuity-coder.config.json');
			const configContent = await readFile('agentuity-coder.config.json', 'utf-8');
			const config = JSON.parse(configContent);
			console.log(chalk.yellow('📁 Existing local configuration found (will migrate to global)!'));
			return config;
		} catch {
			// No existing config
			return null;
...
async function saveConfig(config: ConfigData): Promise<void> {
	...
	const globalConfigPath = getGlobalConfigPath();
	await writeFile(globalConfigPath, JSON.stringify(configToSave, null, 2));

	// Also update .env file in project directory for local development
...
`;
	...
	console.log(chalk.dim(`  ✓ Configuration saved to: ${globalConfigPath}`));
}
...
# Store the actual terminal working directory before changing (only if not already set)
if [ -z "$ORIGINAL_TERMINAL_CWD" ]; then
    export ORIGINAL_TERMINAL_CWD="$PWD"
fi

cd "$(dirname "$0")"

# Read configuration from global location first, then local fallback
GLOBAL_CONFIG="$HOME/.config/agentuity-coder/agentuity-coder.config.json"
if [ -f "$GLOBAL_CONFIG" ]; then
    CONFIG_AGENT_URL=$(node -pe "JSON.parse(require('fs').readFileSync('$GLOBAL_CONFIG')).agentUrl" 2>/dev/null)
    CONFIG_MODE=$(node -pe "JSON.parse(require('fs').readFileSync('$GLOBAL_CONFIG')).mode" 2>/dev/null)
elif [ -f agentuity-coder.config.json ]; then
    CONFIG_AGENT_URL=$(node -pe "JSON.parse(require('fs').readFileSync('agentuity-coder.config.json')).agentUrl" 2>/dev/null)
    CONFIG_MODE=$(node -pe "JSON.parse(require('fs').readFileSync('agentuity-coder.config.json')).mode" 2>/dev/null)
fi

# Also read from .env file for API key
if [ -f .env ]; then
    export $(grep -v '^#' .env | xargs)
fi

# Handle mode selection based on flags and config
if [[ " $* " =~ " --local " ]]; then
    # For local mode, don't set AGENT_URL - let CLI auto-detect
    unset AGENT_URL
    export CODER_MODE="local"
elif [[ " $* " =~ " --cloud " ]]; then
    # For cloud mode, use the URL from config file
    export AGENT_URL="$CONFIG_AGENT_URL"
    export CODER_MODE="cloud"
else
    # Default mode - use config file settings or auto-detect
    if [ "$CONFIG_MODE" = "cloud" ] && [ ! -z "$CONFIG_AGENT_URL" ]; then
        export AGENT_URL="$CONFIG_AGENT_URL"
        export CODER_MODE="cloud"
    else
        unset AGENT_URL
        export CODER_MODE="auto"
    fi
fi
...
# Store original terminal working directory before changing
export ORIGINAL_TERMINAL_CWD="$PWD"
cd "${projectDir}"
exec ./coder "$@"
`;
...
function showCompletionMessage(config: ConfigData) {
	...
	console.log(
		boxen(
			...
			`${chalk.cyan('Configuration saved to:')} ~/.config/agentuity-coder/agentuity-coder.config.json\n` +
	...
	try {
		...
		if (existingConfig) {
			...
			if (action === 'regenerate') {
				console.log(chalk.blue('🔗 Regenerating CLI binary with existing configuration...'));
				...
				console.log(chalk.cyan(`Binary recreated with existing configuration.`));
				return;
			}
