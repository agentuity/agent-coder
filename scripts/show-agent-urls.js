...
async function showAgentUrls() {
  ...
  try {
    ...
    console.log(`  CloudCoder (hybrid): ${chalk.white(localUrls.cloudCoderUrl)}`);
    ...
    console.log('• For cloud deployment, replace "your-agent.agentuity.cloud" with your actual domain');
    console.log('• The agent IDs are automatically detected using "agentuity agent list"');
    
  } catch (error) {
    console.error(chalk.red('❌ Error reading agent configuration:'), error.message);
